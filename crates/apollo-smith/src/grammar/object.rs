...
use apollo_encoder::{ObjectDef, Schema};
use arbitrary::Unstructured;

use crate::{Node, SchemaBuilder};

use super::{description, /*directive,*/ document::is_definition, field, name, ty};
...
pub(crate) fn object_type_definition(u: &mut Unstructured, builder: &mut SchemaBuilder) -> Node {
    let new_name_obj = name::name(u, builder).try_into_name().unwrap();
    builder.type_names.insert(new_name_obj.to_string());
    let mut new_object = ObjectDef::new(new_name_obj);
    ...
    // }
    // ================================
    // Generate description or not
    if u.arbitrary::<bool>().unwrap_or(false) {
        new_object.description(Some(
            description::description(u, builder)
                .try_into_description()
                .expect("unreachable because it's a description"),
        ));
    }

    // Generate name of type
    // TODO Generate implements interface (or not)
    // TODO Generate directives (or not)
    // Generate fields
    builder.field_names.clear();
    let new_field_number = u.arbitrary::<u8>().unwrap() % 100 + 1;
    let mut fields = HashSet::new();
    for _ in 1..=new_field_number {
        let new_field = field::field_definition(u, builder);
        fields.insert(new_field);
    }
    fields
        .into_iter()
        .for_each(|f| new_object.field(f.try_into_field().unwrap()));

    Node::ObjectDef(new_object)
